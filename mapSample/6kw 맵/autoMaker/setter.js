...
const imgObjList = map.MAP.ImgObjList;
...
function calcPlacePoint(placeObjInfo, locatedObjPoint) {
  ...
  if (placeImgType === 'Rect') {
    x =
      x - placeObjInfo.axis[0] * placeImgData.Width + placeObjInfo.xMoveScale * placeImgData.Width;
    y =
      y -
...
function makeObjList(objectList) {
  objectList.forEach(objList => {
    _.forEach(objList, (obj, index) => {
      // BU.CLI(obj)
      const targetPoint = discoverObjectPoint(obj.locatedIdList[0]);
      const finalAxis = calcPlacePoint(obj, targetPoint);
